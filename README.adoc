...
An https://plugins.jetbrains.com/plugin/30958-dependency-assistant[IntelliJ IDEA] plugin that helps you upgrade dependencies declared in Maven, Gradle, GitHub Actions, NPM, and Antora projects, plus Maven and Gradle wrapper properties.
...
When JetBrains Package Checker is available in the IDE, known Maven and npm vulnerabilities can be shown alongside versions.
...
Install the *Dependency Assistant* plugin from the IDE *Settings / Plugins* https://plugins.jetbrains.com/plugin/30958-dependency-assistant[marketplace], or build the distribution locally (see <<Building>>) and install the ZIP from disk.
...
** Alternatively: Choose menu:Tools[Dependency Assistant > Upgrade dependencies…].
