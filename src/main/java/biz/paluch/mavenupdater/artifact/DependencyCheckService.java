...
import biz.paluch.mavenupdater.artifact.xml.PomDependency;
import biz.paluch.mavenupdater.artifact.xml.PomProfile;
import biz.paluch.mavenupdater.artifact.xml.PomProjection;
import biz.paluch.mavenupdater.artifact.xml.XmlBeamProjectorFactory;
...
public class DependencyCheckService {
	...
	private static final Map<ArtifactCoordinate, List<VersionOption>> versionCache = new ConcurrentHashMap<>();
	...
	private void doWithDependency(Projects projects, PomDependency dependency, DeclarationSource declarationSource,
			BiConsumer<ArtifactCoordinate, ArtifactUsage> callback) {
		...
		if (a != null) {
			callback.accept(new ArtifactCoordinate(g, a), new ArtifactUsage(declarationSource, versionSource));
		}
	}
