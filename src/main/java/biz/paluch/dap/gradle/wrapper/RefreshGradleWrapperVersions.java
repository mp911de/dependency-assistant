...
/**
 * Refresh Gradle wrapper versions.
 *
 * @author Mark Paluch
 */
class RefreshGradleWrapperVersions {

	private static final Logger LOG = Logger.getInstance(RefreshGradleWrapperVersions.class);

	private final Cache cache;

	private final Set<ReleaseSource> releaseSources;

	RefreshGradleWrapperVersions(Cache cache, Set<ReleaseSource> releaseSources) {
		this.cache = cache;
		this.releaseSources = releaseSources.isEmpty() ? Set.of(GradleDistributionReleaseSource.INSTANCE)
				: releaseSources;
	}

	void refreshWrapperVersions(ProgressIndicator indicator) {
		...
		ArtifactId artifactId = WrapperProperty.DISTRIBUTION.artifactId();
		indicator.checkCanceled();
		if (cache.getReleases(artifactId).isEmpty()) {
			updateFutures.put(artifactId, executor.submit(() -> releaseResolver.getReleases(artifactId, indicator)));
		}
