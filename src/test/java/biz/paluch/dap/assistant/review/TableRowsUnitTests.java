...
/**
 * Unit tests for {@link UpgradeRows}.
 *
 * @author Mark Paluch
 */
class TableRowsUnitTests {
	...
	void collapsesAgreeingGovernedMembersIntoGroup() {
		...
		UpgradeRows rows = UpgradeRows.builder()
				.add(member(VAVR, "1.0.0", VAVR_RULE))
				.add(member(VAVR_MATCH, "1.0.0", VAVR_RULE))
				.build();

		assertThat(rows).singleElement().isInstanceOfSatisfying(GroupRow.class, group -> {
			assertThat(group.getName()).isEqualTo("Vavr");
			assertThat(group.getCurrentVersion()).isEqualTo(ArtifactVersion.of("1.0.0"));
			assertThat(group.getMembers()).extracting(TableRow::getArtifactId)
	...
	void groupOffersIntersectionOfMemberReleases() {
		...
		assertThat(rows).singleElement().isInstanceOfSatisfying(GroupRow.class, group -> {
			Releases offered = group.getUpgrade().getReleases();
	...
	void selectsLargestVersionAgreeingCohort() {
		...
		assertThat(rows).filteredOn(GroupRow.class::isInstance).singleElement()
				.isInstanceOfSatisfying(GroupRow.class, group -> assertThat(group.getMembers())
						.extracting(TableRow::getArtifactId)
						.containsExactly(SPRING_BOOT.toArtifactId(), SPRING_MODULITH_BOM.toArtifactId()));
		assertThat(rows.toList().getLast()).isNotInstanceOf(GroupRow.class)
				.extracting(TableRow::getArtifactId).isEqualTo(SPRING_DEPENDENCY_MANAGEMENT.toArtifactId());
	}
	...
	void tieBreaksEqualCohortsToHigherVersion() {
		...
		assertThat(rows).filteredOn(GroupRow.class::isInstance).singleElement()
				.isInstanceOfSatisfying(GroupRow.class, group -> {
	...
	void driftingMemberMatchingCohortVersionJoinsGroupCarryingDrift() {
		...
		assertThat(rows).singleElement().isInstanceOfSatisfying(GroupRow.class, group -> {
			assertThat(group.getMembers()).extracting(TableRow::getArtifactId).containsExactly(
	...
	void singleGovernedMemberDoesNotGroupButRelabelsByRuleName() {
		...
		assertThat(rows).noneMatch(GroupRow.class::isInstance);
		assertThat(rows).extracting(TableRow::getName).containsExactly("Vavr", "slf4j-api");
	}
	...
	void leftoverGovernedMemberKeepsCoordinateLabelWhenGroupClaimsRuleName() {
		...
		assertThat(rows).extracting(TableRow::getName)
	...
	void unnamedRuleFallsThroughToInferredGroup() {
		...
		assertThat(rows).singleElement().isInstanceOfSatisfying(GroupRow.class,
				group -> assertThat(group.getName()).isEqualTo("vavr"));
	}
	...
	void sameRuleNameAcrossEcosystemsNeverGroups() {
		...
		assertThat(rows).hasSize(2).noneMatch(GroupRow.class::isInstance);
	}
	...
	void placesGroupAtFirstMemberPreservingSurroundingOrder() {
		...
		List<TableRow> ordered = rows.toList();
		assertThat(ordered).hasSize(3);
		assertThat(ordered.get(0)).isNotInstanceOf(GroupRow.class)
				.extracting(TableRow::getArtifactId).isEqualTo(SLF4J_API.toArtifactId());
		assertThat(ordered.get(1)).isInstanceOf(GroupRow.class);
		assertThat(ordered.get(2)).isNotInstanceOf(GroupRow.class)
				.extracting(TableRow::getArtifactId).isEqualTo(GUAVA.toArtifactId());
	}

	@Test
	void passesThroughUngovernedCandidatesUnchanged() {

		TableRow slf4j = member(SLF4J_API, "2.0.17", DependencyRule.absent());
		TableRow guava = member(GUAVA, "33.6.0-jre", DependencyRule.absent());

		UpgradeRows rows = UpgradeRows.builder().add(slf4j).add(guava).build();
	...
	void collapsesUngovernedPrefixFamilyWithDerivedLabel() {
		...
		assertThat(rows).singleElement().isInstanceOfSatisfying(GroupRow.class, group -> {
			assertThat(group.getName()).isEqualTo("spring-data");
			assertThat(group.getMembers()).extracting(TableRow::getArtifactId).containsExactly(
	...
	void separatesSamePrefixAcrossDifferentGroupIds() {
		...
		assertThat(rows).hasSize(2).allMatch(GroupRow.class::isInstance);
		assertThat(rows).extracting(TableRow::getName)
	...
	void inferredGroupCollapsesCoReleasedMembers() {
		...
		assertThat(rows).singleElement().isInstanceOfSatisfying(GroupRow.class,
				group -> assertThat(group.getMembers()).extracting(TableRow::getArtifactId).containsExactly(
	...
	void inferredGroupSplitsDistinctReleaseLinesIntoSeparateGroups() {
		...
		assertThat(rows).hasSize(2).allMatch(GroupRow.class::isInstance);
		assertThat(rows).extracting(TableRow::getName).containsExactly("foo-client", "foo-server");
	}
	...
	void singleUngovernedCandidateStaysIndividualRow() {
		...
		assertThat(rows).singleElement().isNotInstanceOf(GroupRow.class);
	}
	...
	void governedAndInferredGroupsCoexist() {
		...
		assertThat(rows).hasSize(2).allMatch(GroupRow.class::isInstance);
		assertThat(rows).extracting(TableRow::getName).containsExactly("Vavr", "spring-data");
	}
	...
	private static TableRow ungoverned(String groupId, String artifactId, String version) {
		return ungoverned(groupId, artifactId, DependencyRule.absent(), null, version);
	}

	private static TableRow ungoverned(String groupId, String artifactId, @Nullable String property,
			String... versions) {
		return ungoverned(groupId, artifactId, DependencyRule.absent(), property, versions);
	}

	private static TableRow ungoverned(String groupId, String artifactId, DependencyRule rule, String version) {
		return ungoverned(groupId, artifactId, rule, null, version);
	}

	private static TableRow ungoverned(String groupId, String artifactId, DependencyRule rule,
		...
		return new TableRow(
				DependencyUpgradeCandidate.create(dependency, TestReleases.from(versions[0]),
						VulnerabilityRepository.empty(), rule, TestInterfaceAssistant.INSTANCE,
						DeclaredVersions.from(declarationSites(id, versions), it -> null, null)));
	}

	private static TableRow released(String groupId, String artifactId, String current,
			String... releaseVersions) {

		return released(groupId, artifactId, DependencyRule.absent(), current, releaseVersions);
	}

	private static TableRow released(String groupId, String artifactId, DependencyRule rule, String current,
			String... releaseVersions) {
		...
		return new TableRow(
				DependencyUpgradeCandidate.create(dependency, TestReleases.from(releaseVersions),
						VulnerabilityRepository.empty(), rule, TestInterfaceAssistant.INSTANCE,
						DeclaredVersions.from(List.of(site), it -> null, null)));
	}

	private static TableRow member(CachedArtifact artifact, String version, DependencyRule rule) {
	...
	private static TableRow driftingMember(CachedArtifact artifact, DependencyRule rule, String... versions) {
		return candidate(artifact, rule, TestInterfaceAssistant.INSTANCE, null, versions);
	}

	private static TableRow propertyMember(CachedArtifact artifact, DependencyRule rule, String property,
			String... versions) {
		return candidate(artifact, rule, TestInterfaceAssistant.INSTANCE, property, versions);
	}

	private static TableRow candidate(CachedArtifact artifact, DependencyRule rule,
			InterfaceAssistant assistant,
			@Nullable String property, String... declaredVersions) {

		ArtifactId id = artifact.toArtifactId();
		DeclaredVersions declarations = DeclaredVersions.from(declarationSites(id, declaredVersions), it -> null, null);
		Dependency dependency = new Dependency(id, declarations.getLowestDeclaredVersion());
		dependency.addDeclarationSource(DeclarationSource.dependency());
		dependency.addVersionSource(property != null ? VersionSource.property(property)
		...
		return new TableRow(
				DependencyUpgradeCandidate.create(dependency, releases, VulnerabilityRepository.empty(), rule,
						assistant,
						declarations));
	}
