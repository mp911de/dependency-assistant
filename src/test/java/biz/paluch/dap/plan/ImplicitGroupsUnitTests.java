...
class ImplicitGroupsUnitTests {
	...
	void unrelatedCapturesStayTopLevel() {
		...
		Map<UpgradePlanSource, ArtifactVersion> upgrades = new LinkedHashMap<>();
		upgrades.put(new TestUpgradePlanSource(
		...
		Sequence<Upgrade> items = ImplicitGroups.create(upgrades, new ApplicationSettings());

		assertThat(items).extracting(Upgrade::getDisplayName).containsExactly("spring-core", "jackson-core");
	...
	void rememberedNameWinsForMatchingConstellation() {
		...
		Map<UpgradePlanSource, ArtifactVersion> upgrades = new LinkedHashMap<>();
		upgrades.put(new TestUpgradePlanSource(candidate("org.springframework:spring-core:6.2.1")),
				ArtifactVersion.of("6.2.2"));
		upgrades.put(new TestUpgradePlanSource("jackson", List.of(
				candidate("com.fasterxml.jackson.core:jackson-core:2.18.0"),
		...
		assertThat(ImplicitGroups.create(upgrades, settings)).extracting(Upgrade::getDisplayName)
	...
	void divergingPinnedTargetsStaySeparate() {
		...
		assertThat(items).extracting(Upgrade::getDisplayName).containsExactly("spring-core", "addon");
		assertThat(items).allSatisfy(item -> assertThat(item.getMembers()).hasSize(1)
				.noneMatch(member -> member.implicit));
	}
	...
	void mixedSourcePeerRetainsInlineDeclaration() {
		...
		upgrades.put(new TestUpgradePlanSource(
				candidate("org.springframework:spring-core:6.2.1", it -> it.versionProperty("spring.version"))),
				ArtifactVersion.of("6.2.2"));
		upgrades.put(new TestUpgradePlanSource(candidate("com.example:addon:6.2.1",
		...
		assertThat(items).singleElement().satisfies(item -> {
			...
			Dependency retained = item.getMembers().getLast();
	...
	void multiPropertyPeerBelongsToFirstOwnerWithoutMergingGroups() {
		...
		TestUpgradePlanSource jackson = new TestUpgradePlanSource("Jackson", List.of(
				candidate("com.fasterxml.jackson.core:jackson-core:2.18.0",
						it -> it.versionProperty("jackson.version")),
				candidate("com.fasterxml.jackson.core:jackson-databind:2.18.0",
						it -> it.versionProperty("jackson.version"))));
		TestUpgradePlanSource micrometer = new TestUpgradePlanSource("Micrometer", List.of(
		...
		upgrades.put(new TestUpgradePlanSource(candidate("com.example:addon:2.18.0",
		...
		List<Upgrade> items = ImplicitGroups.create(upgrades, new ApplicationSettings()).toList();

		assertThat(items).extracting(Upgrade::getDisplayName).containsExactly("Jackson", "Micrometer");
	...
	private static TestUpgradePlanSource springGroup() {
		return new TestUpgradePlanSource("Spring", List.of(
				candidate("org.springframework:spring-core:6.2.1", it -> it.versionProperty("spring.version")),
